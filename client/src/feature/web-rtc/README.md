...
- 예를 들어, websocket을 사용 시 클라이언트는 서버한테 메시지를 보내는 것이고, 그 서버가 클라이언트에게 다시 메시지를 보내는 것이다. 하지만 webRTC는 시그널링 이후에 peer to peer로 연결된다.

# 2. WebRTC의 Media Streams API
...
- 시그널링 서버는 WebRTC 통신에서 두 피어 간의 실시간 미디어 연결을 위한 중개자 역할을 한다. 이는 피어 간 메타데이터와 세션 제어 정보를 교환하는 과정인 시그널링을 통해 이루어지며, SDP(세션 설명 프로토콜) 요청 및 응답, 후보 IP 주소와 같은 초기 연결 설정 정보, 코덱 및 프로토콜 협상, 세션 시작 및 종료 처리 등의 정보를 전달한다. WebRTC는 특정 시그널링 프로토콜을 명시하지 않으므로 개발자는 WebSocket, HTTP, XMPP, SIP 등 다양한 방법을 활용해 시그널링 과정을 구현할 수 있다. 이를 통해 피어들이 서로를 발견하고 통신 경로를 확보함으로써 인터넷 상에서의 실시간 오디오 및 비디오 전송을 가능하게 한다.
