...
- SDP(Session Description Protocol)는 실시간 통신 시스템에서 세션 정보(양쪽 피어 간의 연결 및 미디어 교환을 설정하는 데 필요한 데이터를 의미한다. 이 정보는 두 피어가 어떤 형식으로, 어떤 조건에서, 어떤 경로를 통해 데이터를 주고받을지를 정의한다)를 설명하고 교환하는 데 사용되는 텍스트 기반의 표준 포맷이다. 주로 WebRTC와 같은 P2P 통신에서 사용되며, 미디어 스트림 교환을 위해 필요한 메타데이터와 네트워크 정보를 포함한다. 예를 들어, 비디오의 해상도, 코덱, 오디오 전송 여부 등과 같은 초기 설정 정보와 IP 주소, 포트, ICE 후보 등 네트워크 경로 관련 정보 등이 있다.

- SDP는 프로토콜이 아닌 데이터 포맷이며, WebRTC의 **Offer/Answer** 모델을 기반으로 작동한다. 한 피어가 세션 정보를 포함한 Offer를 전송하면, 상대 피어는 이를 수락하거나 수정하여 Answer를 반환한다. 이 과정에서 각 피어는 ICE 후보(네트워크 경로)를 수집하고, 최적의 ICE 후보를 선택해 가장 안정적이고 지연 시간이 적은 경로를 결정한다.

- 최적의 경로가 설정되면, 피어 간 필요한 모든 메타데이터와 네트워크 정보에 대해 합의가 완료되고, 미디어 스트림 전송이 시작된다. SDP는 P2P 통신에서 초기 세션 설정과 네트워크 연결 협상을 책임지는 역할을 한다.
